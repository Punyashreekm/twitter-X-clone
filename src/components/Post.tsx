...
const Post = async ({ type }: { type?: "status" | "comment" }) => {
  ...
  const fileDetails = await getFileDetails("677d28dc432c476416e59f89");
      ...
      <div className={`flex gap-4 ${type === "status" && "flex-col"}`}>
        ...
        <div className="flex-1 flex flex-col gap-2">
          ...
          <div className="w-full flex justify-between">
            <Link href={`/punyaWebDev`} className="flex gap-4">
              <div className={`${type !== "status" && "hidden"} relative w-10 h-10 rounded-full overflow-hidden`}>
                <Image path="public/general/avatar.png" alt="" h={100} w={100} tr={true} />
              </div>
              <div
              ...
              >
                <h1 className="text-md font-bold">Punya</h1>
                <span className={`text-textGray ${type === "status" && "text-sm"}`}>@PunyaWewDev</span>
